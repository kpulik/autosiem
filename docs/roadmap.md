...
- [x] Standards / threat-intel update cycle — `autosiem.update_job` (`UpdateJob.run_once`) wired as CLI `update`: reloads local rules, recomputes coverage, refreshes STIX intel from a configured URL or path, and refreshes the **ATT&CK matrix** from MITRE's published releases via `--refresh-attack` (version-checks first, downloads only on a new release, HTTPS enforced). Not hourly by itself — the `schedule()` daemon is a library API nothing starts, so run it from cron. Detection rules are still never fetched over the network; automatic rule updates (SigmaHQ sync) are open work.
