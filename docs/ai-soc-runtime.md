...
- **Update job** (`autosiem.update_job`) — `UpdateJob.run_once()` reloads rules from the local rule directory, recomputes ATT&CK coverage, and returns an `UpdateReport` — wired as CLI `update`. Two network steps are available and **both are opt-in, off by default**: threat intel from `--intel-url`/`--intel-path`, and the ATT&CK matrix from `--refresh-attack`, which checks MITRE's published release list first and downloads the ~54 MB bundle only when a newer version exists. Coverage is then reported against the refreshed matrix. Still true: **detection rules are only ever re-read from disk, never fetched over the network**, and while `schedule()`/`start()` provide a daemon thread as a library API, nothing in the shipped code starts one — run `cli update` from cron or a systemd timer.
