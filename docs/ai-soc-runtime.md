...
- **Update job** (`autosiem.update_job`) — `UpdateJob.run_once()` reloads rules from the local rule directory, recomputes ATT&CK coverage, refreshes threat intel from an `--intel-url` or `--intel-path` when one is configured, and returns an `UpdateReport` — wired as CLI `update`. Two limits worth stating: **rules are only ever re-read from disk, never fetched over the network**, and while `schedule()`/`start()` provide an hourly daemon thread as a library API, nothing in the shipped code starts one. For periodic refresh, run `cli update` from cron or a systemd timer.
