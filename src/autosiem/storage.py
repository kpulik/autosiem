...
class AutoSIEMStorage:
    ...
    def save_pipeline_result(self, result: Any, tenant_id: str | None = None) -> None:
        ...
        with self.connect() as conn:
            for event in result.events:
                ...
                conn.execute(
                    """
                    insert or replace into events(event_id,timestamp,category,action,user,host,src_ip,severity,data,tenant_id)
                    values(?,?,?,?,?,?,?,?,?,?)
                    """,
                    (
                        event.event_id,
            ...
            for finding in result.findings:
                ...
                conn.execute(
                    """
                    insert or replace into findings(finding_id,rule_id,rule_name,event_id,timestamp,severity,risk_points,data,tenant_id)
                    values(?,?,?,?,?,?,?,?,?)
                    """,
                    (
                        finding.finding_id,
            ...
            for incident in result.incidents:
                ...
                conn.execute(
                    """
                    insert or replace into incidents(incident_id,title,severity,risk_score,status,created_at,data,tenant_id)
                    values(?,?,?,?,?,?,?,?)
                    """,
                    (
                ...
                if investigation:
                    ...
                    if note:
                        comment = self._add_comment(conn, incident.incident_id, AI_ANALYST_ACTOR, note)
                        self.audit(
                            conn,
                            actor=AI_ANALYST_ACTOR,
                            action="incident_commented",
                            target=incident.incident_id,
                            details={"comment_id": comment["comment_id"], "source": "ai_case_note"},
                        )
            self.audit(conn, actor="system", action="pipeline_result_saved", target=None, details={"events": len(result.events), "findings": len(result.findings), "incidents": len(result.incidents)})
    ...
    def _save_investigation(
        ...
        conn.execute(
            """
            insert or replace into investigations(investigation_id,incident_id,status,decision,confidence,created_at,data,tenant_id)
            values(?,?,?,?,?,?,?,?)
            """,
            (
        ...
        for proposal in investigation.action_proposals:
            conn.execute(
                ...
                insert or replace into action_proposals(
                    proposal_id,investigation_id,incident_id,action,target,confidence,approval_required,executable_now,status,data,tenant_id
                ) values(?,?,?,?,?,?,?,?,?,?,?)
                """,
                (
        ...
        with self.connect() as conn:
            if tenant_id:
                incident = conn.execute(
                    "select * from incidents where incident_id = ? and tenant_id = ?",
            ...
            investigation = conn.execute("select * from investigations where incident_id = ?", (incident_id,)).fetchone()
            proposals = conn.execute("select * from action_proposals where incident_id = ? order by action", (incident_id,)).fetchall()
            finding_ids = incident_doc.get("data", {}).get("finding_ids", [])
            findings = self._findings_by_ids(conn, finding_ids)
            event_ids = [finding["event_id"] for finding in findings]
            events = self._events_by_ids(conn, event_ids)
            return {
                ...
                "comments": self._list_comments(conn, incident_id),
            }
    ...
    def search_events(self, query: str | None = None, entity: str | None = None, limit: int = 100, tenant_id: str | None = None) -> list[dict[str, Any]]:
        ...
        if query:
            like = f"%{query}%"
            where.append("(event_id like ? or category like ? or action like ? or user like ? or host like ? or src_ip like ? or data like ?)")
    ...
    def search_incidents(self, query: str | None = None, entity: str | None = None, status: str | None = None, limit: int = 50, tenant_id: str | None = None) -> list[dict[str, Any]]:
        ...
        if query:
            like = f"%{query}%"
            where.append("(incident_id like ? or title like ? or severity like ? or data like ?)")
    ...
    def _findings_by_ids(self, conn: sqlite3.Connection, finding_ids: list[str]) -> list[dict[str, Any]]:
        if not finding_ids:
            return []
        placeholders = ",".join("?" for _ in finding_ids)
        rows = conn.execute(f"select * from findings where finding_id in ({placeholders}) order by timestamp", finding_ids).fetchall()
        return [_row_to_dict(row) for row in rows]

    def _events_by_ids(self, conn: sqlite3.Connection, event_ids: list[str]) -> list[dict[str, Any]]:
        if not event_ids:
            return []
        placeholders = ",".join("?" for _ in event_ids)
        rows = conn.execute(f"select * from events where event_id in ({placeholders}) order by timestamp", event_ids).fetchall()
        return [_row_to_dict(row) for row in rows]
        ...
        with self.connect() as conn:
            if tenant_id:
                proposal = conn.execute(
                    "select * from action_proposals where proposal_id = ? and tenant_id = ?",
                    (proposal_id, tenant_id),
                ).fetchone()
            else:
                proposal = conn.execute("select * from action_proposals where proposal_id = ?", (proposal_id,)).fetchone()
            if not proposal:
                return None
            conn.execute("update action_proposals set status = ? where proposal_id = ?", (decision, proposal_id))
            self.audit(
                conn,
                actor=actor,
    ...
    def update_incident(
        ...
        with self.connect() as conn:
            ...
            conn.execute(
                "update incidents set status = ?, assignee = ?, resolution = ?, updated_at = ? where incident_id = ?",
                (current["status"], current["assignee"], current["resolution"], current["updated_at"], incident_id),
            )
            if note:
                self._add_comment(conn, incident_id, actor, note)
            changed = {"status": current["status"], "assignee": current["assignee"], "resolution": current["resolution"]}
            self.audit(conn, actor=actor, action="incident_updated", target=incident_id, details=changed)
            return _row_to_dict(conn.execute("select * from incidents where incident_id = ?", (incident_id,)).fetchone())
    ...
    def add_incident_comment(self, incident_id: str, actor: str, body: str, tenant_id: str | None = None) -> dict[str, Any] | None:
        with self.connect() as conn:
            if tenant_id:
                exists = conn.execute(
                    "select 1 from incidents where incident_id = ? and tenant_id = ?",
                    (incident_id, tenant_id),
                ).fetchone()
            else:
                exists = conn.execute("select 1 from incidents where incident_id = ?", (incident_id,)).fetchone()
            if not exists:
                return None
            comment = self._add_comment(conn, incident_id, actor, body)
            self.audit(conn, actor=actor, action="incident_commented", target=incident_id, details={"comment_id": comment["comment_id"]})
            return comment
    ...
    def _add_comment(self, conn: sqlite3.Connection, incident_id: str, actor: str, body: str) -> dict[str, Any]:
        comment = {
    ...
    def _list_comments(self, conn: sqlite3.Connection, incident_id: str, tenant_id: str | None = None) -> list[dict[str, Any]]:
    ...
    def audit(self, conn: sqlite3.Connection, actor: str, action: str, target: str | None, details: dict[str, Any]) -> None:
